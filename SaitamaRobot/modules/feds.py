...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @OneKickSupport if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @OneKickSupport should this problem persist!"
            )
            return
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        fed_name = info['fname']
        #https://t.me/OneKickSupport/41606 // https://t.me/OneKickSupport/41619
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @OneKickSupport."
            )
            return
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @OneKickSupport !"
            )
            return
