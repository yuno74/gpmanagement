...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text(
                 "Trying to put me against a Mythic level disaster huh?")
            return log_message
            ...
            message.reply_text(
                "Fighting this Epic here will put civilian lives at risk.")
            return log_message
        elif user_id in DEMONS:
            message.reply_text(
                "Bring an order from Legend to fight a GrandMaster."
            )
            return log_message
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from Legend to fight a Master."
            )
            return log_message
        elif user_id in WOLVES:
            message.reply_text("Elite abilities make them ban immune!")
            return log_message
        else:
...
*Admins only:*
 ...
 • `/kick <userhandle>`*:* kicked a user out of the group, (via handle, or reply)
"""
